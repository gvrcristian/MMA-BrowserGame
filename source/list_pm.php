...
$req1 = mysql_query('select m1.id, m1.title, m1.timestamp, count(m2.id) as reps, users.id as user_id, users.usern from pm as m1, pm as m2,users where ((m1.user1="'.$_SESSION['user_id'].'" and m1.user1read="no" and users.id=m1.user2) or (m1.user2="'.$_SESSION['user_id'].'" and m1.user2read="no" and users.id=m1.user1)) and m1.id2="1" and m2.id=m1.id group by m1.id order by m1.id desc');
$req2 = mysql_query('select m1.id, m1.title, m1.timestamp, count(m2.id) as reps, users.id as user_id, users.usern from pm as m1, pm as m2,users where ((m1.user1="'.$_SESSION['user_id'].'" and m1.user1read="yes" and users.id=m1.user2) or (m1.user2="'.$_SESSION['user_id'].'" and m1.user2read="yes" and users.id=m1.user1)) and m1.id2="1" and m2.id=m1.id group by m1.id order by m1.id desc');
?>

<a href="./?m=mes" class="button transition">New PM</a><br /><br>
<b>Unread Messages(<?php echo intval(mysql_num_rows($req1)); ?>):</b>
<table  align="center" width="70%" class="ms">
	<tr>
...
?>
	<tr>
    	<td><a href="./?m=read&id=<?php echo $dn1['id']; ?>"><?php echo htmlentities($dn1['title'], ENT_QUOTES, 'UTF-8'); ?></a></td>
    	<td><?php echo $dn1['reps']-1; ?></td>
    	<td><a href="#"><?php echo htmlentities($dn1['usern'], ENT_QUOTES, 'UTF-8'); ?></a></td>
...
<br />
<b>Read Messages(<?php echo intval(mysql_num_rows($req2)); ?>):</b>
<table  align="center" width="70%"class="ms">
	<tr>
    	<th class="title_cell">Title</th>
        <th>Nb. Replies</th>
...
?>
	<tr>
    	<td ><a href="./?m=read&id=<?php echo $dn2['id']; ?>"><?php echo htmlentities($dn2['title'], ENT_QUOTES, 'UTF-8'); ?></a></td>
    	<td><?php echo $dn2['reps']-1; ?></td>
    	<td><a href="#"><?php echo htmlentities($dn2['usern'], ENT_QUOTES, 'UTF-8'); ?></a></td>
